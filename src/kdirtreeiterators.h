...
protected:
   ...
   **/
  KFileInfoIterator(KFileInfo *parent, KDotEntryPolicy dotEntryPolicy,
                    bool callNext);
...
private:
   ...
   **/
  void init(KFileInfo *parent, KDotEntryPolicy dotEntryPolicy, bool callNext);
...
public:
  ...
  KFileInfoSortedBySizeIterator(
      KFileInfo *parent, KFileSize minSize = 0,
      KDotEntryPolicy dotEntryPolicy = KDotEntryTransparent,
 ...
 * class.
 **/
template <class T> inline int compare(T val1, T val2) {
  if (val1 < val2)
    return -1;
  else if (val1 == val2)
    return 0;
