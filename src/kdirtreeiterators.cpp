...
void KFileInfoIterator::init(KFileInfo *parent, KDotEntryPolicy dotEntryPolicy,
                             bool callNext) {
  _parent = parent;
    ...
    } else // Dot entry already processed or processing it not desired
    {
      if (!_dotEntryChildrenProcessed) {
        if (_policy == KDotEntryTransparent) {
          ...
          _current = _current ? _current->next()
                              : (_parent->dotEntry()
                                     ? _parent->dotEntry()->firstChild()
                                     : 0);

          if (!_current) {
            _dotEntryChildrenProcessed = true;
          } else {
            // qDebug() << Q_FUNC_INFO << " dot entry child " << _current <<
            // endl;
          }
...
void KFileInfoSortedIterator::delayedInit() {
...
void KFileInfoSortedIterator::makeChildrenList() {
...
KFileInfo *KFileInfoSortedIterator::current() {
  if (!_initComplete)
    delayedInit();
...
bool KFileInfoSortedIterator::finished() {
  if (!_initComplete)
    delayedInit();
...
KFileInfoSortedBySizeIterator::KFileInfoSortedBySizeIterator(
    KFileInfo *parent, KFileSize minSize, KDotEntryPolicy dotEntryPolicy,
    bool ascending)
    : KFileInfoSortedIterator(parent, dotEntryPolicy, KSortByTotalSize,
                              ascending),
      _minSize(minSize) {}

void KFileInfoSortedBySizeIterator::makeChildrenList() {
  KFileInfoIterator it(_parent, _policy);
...
KFileSize KFileInfoList::sumTotalSizes() {
  KFileSize sum = 0;
